...
import { Plus, Trash2, Clock, Mail, Loader2, Save, Zap, Play, Settings, ArrowDown, Filter, Tag, Webhook, UserCheck, ChevronDown, X, ToggleLeft, ToggleRight } from "lucide-react";

const TRIGGERS = [
  { id: "RESERVATION_CREATED", label: "Nueva Reserva", icon: "📋", color: "#3B82F6" },
  { id: "RESERVATION_CONFIRMED", label: "Pago Confirmado", icon: "✅", color: "#10B981" },
...
const STEP_TYPES = [
  ...
  { id: "wait", label: "Esperar", icon: Clock, color: "#F59E0B", desc: "Pausar X días antes del siguiente paso" },
  { id: "condition", label: "Condición", icon: Filter, color: "#8B5CF6", desc: "Continuar solo si se cumple" },
...
export default function AutomatizacionesPage() {
  ...
  function handleCreate() {
    setEditing({
      ...
      description: "",
      active: true,
      steps: [{ type: "email", templateKey: "WELCOME", label: "Email de bienvenida" }]
    });
  }
  ...
  function addStep(type: string) {
    ...
    setEditing({ ...editing, steps: [...editing.steps, defaults[type] || defaults.email] });
  ...
  if (!editing) return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-10">
        ...
        <button onClick={handleCreate} className="bg-zinc-900 dark:bg-white dark:text-zinc-900 text-white px-5 py-2.5 rounded-lg flex items-center gap-2 font-bold hover:scale-105 transition-transform">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {workflows.map(w => {
          ...
          return (
            <div key={w.id} onClick={() => setEditing(w)} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6 rounded-2xl shadow-sm hover:shadow-md hover:border-zinc-300 dark:hover:border-zinc-700 transition-all cursor-pointer group">
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-2">
                  <span className="text-lg">{trigger?.icon || "⚡"}</span>
                  <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${w.active ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500'}`}>{w.active ? 'Activo' : 'Pausado'}</span>
                </div>
              ...
              <p className="text-sm text-zinc-500 mb-5">{trigger?.label || w.trigger}</p>
                  ...
                  return (
                    <div key={i} className="flex items-center gap-1.5">
                      <div className="w-7 h-7 rounded-lg flex items-center justify-center text-white text-xs" style={{ background: meta?.color || "#999" }}>
                        {s.type === "email" ? "✉" : s.type === "wait" ? "⏳" : s.type === "condition" ? "?" : s.type === "tag" ? "🏷" : "⚡"}
                      </div>
                      {i < Math.min(steps.length - 1, 4) && <ArrowDown className="w-3 h-3 text-zinc-300 rotate-[-90deg]" />}
                    </div>
                  );
                })}
                {steps.length > 5 && <span className="text-[10px] text-zinc-400 ml-1">+{steps.length - 5}</span>}
              </div>
            </div>
          );
        })}
        {workflows.length === 0 && (
          <div className="md:col-span-2 py-24 text-center border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-2xl">
            ...
            <p className="text-zinc-400 text-sm">Crea tu primer flujo para empezar a automatizar emails.</p>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-lg overflow-hidden mb-8">
        <div className="p-5 border-b border-zinc-100 dark:border-zinc-800 flex items-center justify-between bg-zinc-50/50 dark:bg-zinc-800/30">
          <div className="flex items-center gap-4">
            ...
            <input type="text" value={editing.name} onChange={e => setEditing({...editing, name: e.target.value})} className="bg-transparent text-xl font-bold focus:outline-none w-64" />
          </div>
      ...
      <div className="flex flex-col items-center">
        {/* Trigger Node */}
        <div className="w-full max-w-md bg-white dark:bg-zinc-900 border-2 rounded-2xl shadow-lg overflow-hidden" style={{ borderColor: trigger?.color || "#3B82F6" }}>
          <div className="px-5 py-3 flex items-center gap-3" style={{ background: `${trigger?.color}15` }}>
            <span className="text-2xl">{trigger?.icon}</span>
            <div className="flex-1">
              <div className="text-[10px] font-bold uppercase tracking-widest" style={{ color: trigger?.color }}>Disparador</div>
              <select value={editing.trigger} onChange={e => setEditing({...editing, trigger: e.target.value})} className="bg-transparent font-bold text-sm focus:outline-none w-full mt-0.5">
                {TRIGGERS.map(t => <option key={t.id} value={t.id}>{t.label}</option>)}
              </select>
            </div>
          </div>
          <div className="px-5 py-3 border-t border-zinc-100 dark:border-zinc-800">
            <input type="text" placeholder="Descripción del flujo (opcional)" value={editing.description || ""} onChange={e => setEditing({...editing, description: e.target.value})} className="bg-transparent text-xs text-zinc-500 focus:outline-none w-full" />
          </div>
        </div>
            ...
            <div key={i} className="flex flex-col items-center w-full max-w-md">
              ...
              <div className="w-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm hover:shadow-md transition-shadow group relative">
                <div className="absolute -top-2 -right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => removeStep(i)} className="w-6 h-6 bg-red-500 text-white rounded-full flex items-center justify-center shadow-md hover:bg-red-600"><X className="w-3 h-3" /></button>
                </div>
                <div className="p-4 flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                      <select value={step.type} onChange={e => updateStep(i, { type: e.target.value })} className="bg-transparent font-bold text-sm focus:outline-none">
                    ...
                    {step.type === "email" && (
                      <input type="text" placeholder="Plantilla (ej: D14, WELCOME)" value={step.templateKey || ""} onChange={e => updateStep(i, { templateKey: e.target.value })} className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg px-3 py-2 text-sm" />
                    )}
                    {step.type === "wait" && (
                      <div className="flex items-center gap-3">
                        <input type="number" min="0" value={step.delayDays || 0} onChange={e => updateStep(i, { delayDays: parseInt(e.target.value) || 0 })} className="w-16 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg px-3 py-2 text-sm text-center" />
                        <span className="text-sm text-zinc-500">días de espera</span>
                      </div>
                    )}
                    {step.type === "condition" && (
        ...
        <div className="relative">
          <button onClick={() => setShowStepPicker(!showStepPicker)} className="w-10 h-10 rounded-full border-2 border-dashed border-zinc-300 dark:border-zinc-700 flex items-center justify-center text-zinc-400 hover:text-zinc-900 hover:border-zinc-500 dark:hover:text-white dark:hover:border-zinc-500 transition-all">
            <Plus className="w-5 h-5" />
          </button>
          {showStepPicker && (
            <div className="absolute top-12 left-1/2 -translate-x-1/2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-2xl p-3 w-72 z-10">
              ...
              {STEP_TYPES.map(st => (
                <button key={st.id} onClick={() => addStep(st.id)} className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors text-left">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center text-white" style={{ background: st.color }}><st.icon className="w-4 h-4" /></div>
                  <div>
