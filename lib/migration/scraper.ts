...
export async function scrapeGastroshowsPage(url: string): Promise<ScrapingResult> {
  ...
  try {
    ...
    if (process.env.MIGRATION_SECRET_TOKEN) {
      headers['X-Migration-Token'] = process.env.MIGRATION_SECRET_TOKEN;
    }
    ...
    return {
      success: true,
      message: `Página "${title}" importada con éxito.`,
      pageTitle: title,
      slug
    };
    ...
    return {
      success: false,
      message: `Error al importar la página: ${err.message}`
    };
  }
}
