...
export function HeaderBlock({ id, content, isEditing = false, onUpdate, onSelectElement, selectedElementPath }: Props) {
  ...
  const headerStyle: React.CSSProperties = {
    ...
    padding: isScrolled ? "0.9rem 2rem" : (customStyles.padding || "1.3rem 2rem"),
    backgroundColor: isTransparent && !isScrolled && !isMenuOpen ? "transparent" : (isScrolled || isMenuOpen ? "rgba(255, 255, 255, 0.98)" : (customStyles.backgroundColor || "white")),
    backdropFilter: (isScrolled || !isTransparent || isMenuOpen) ? (customStyles.backdropFilter || "blur(8px)") : "none",
    borderBottom: isScrolled ? "1px solid rgba(0,0,0,0.04)" : (customStyles.borderBottom || "none"),
